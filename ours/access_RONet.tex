...
\title{RO-Net: Multimodal Stacked Bidirectional LSTM with Attentions for Indoor Localization using Range-only Measurements}
...
In this section, we briefly survey previous researches closely focused on localization using range measurements and case study on utilizing deep learning for localization, especially about applications of Recurrent Neural Networks(RNNs). Fianlly, we examine how the attention is introduced in the deep learning architectures. 

\subsection{Localization using range measurements}

 Various kinds of sensors have been utilized to localize a object using range measurement sensors, such as GPS, ultrasonic-based sensors. ultra-wideband(UWB) sensors. However, almost distance measured by range measurement sensors are based on Time of Flight(TOF), Time of Arriaval(TOA)\cite{jung2011indoor}, or Time of Differential Arrival(TDOA).  
